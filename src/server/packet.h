...
struct output_header_t
{
  ...
  float ateam[4];                   ///< Ateam fluxes (CasA, CygA, Tau, Vir)
  std::bitset<64> flagged_channels; ///< bitset of flagged channels (8b)
  std::bitset<576> flagged_dipoles; ///< bitset of flagged dipoles (72b)
  uint32_t weights[87];             ///< station weights n*(n+1) / 2, where n in {6, 12}
  uint8_t pad[512-484];             ///< 512 byte block
};
